...
def parse_request(request: service_pb2.PostModelOutputsRequest):
  ...
  byte_images  = [inp.data.image.base64 for inp in request.inputs]
  ...
  messages = []
  for prompt, image in zip_longest(prompts, byte_images):
    try:
...
class MyRunner(ModelRunner):
  ...
  def predict(self, request: service_pb2.PostModelOutputsRequest
    ...
    if not isinstance(list_generated_text, list):
      list_generated_text = [list_generated_text]
